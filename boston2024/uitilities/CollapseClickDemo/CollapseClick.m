...
//        }
        ...
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(iconForTitle:)]) {
            [cell.TitleIcon setImage: [CollapseClickDelegate iconForTitle:xx]];
        }
        else
            cell.TitleIcon = nil;
    }
...
-(void)openCellResize:(int)index
{
    ...
    //New height with header section buttons
//    newSize.height = self.frame.size.height + cell.ContentView.frame.size.height+kCCHeaderHeight;
    
    //New height withut header section buttons
    newSize.height = self.frame.size.height + cell.ContentView.frame.size.height;
...
{
    [UIView animateWithDuration:0.25 animations:^{
        //frame with header section buttons
//        self.superview.frame = CGRectMake(self.frame.origin.x, (768-originalFrameSize.height-kCCHeaderHeight)/2, self.originalFrameSize.width, self.originalFrameSize.height+kCCHeaderHeight);
        ...
        //frame without header section buttons
//        self.superview.frame = CGRectMake(self.frame.origin.x, (768-originalFrameSize.height-kCCHeaderHeight)/2, self.originalFrameSize.width, self.originalFrameSize.height);
        self.superview.frame = CGRectMake(self.frame.origin.x, (768-originalFrameSize.height)/2, self.originalFrameSize.width, self.originalFrameSize.height);
...
//            cell.TitleSideBar.alpha = 0.0;
        ...
        theCell.TitleLabel.textColor = [UIColor whiteColor];
    }
}
